...
def supabase_request(endpoint, method="GET", data=None):
    ...
    try:
        ...
        if response.status_code in [200, 201, 204]: return response.json() if response.text else []
        else: st.error(f"Supabase Error {response.status_code}: {response.text}"); return []
    except Exception as e: st.error(f"Supabase Error: {e}"); return []

def load_all_warung(): return supabase_request("warung?order=id.asc")
def save_warung(warung):
    if warung.get('id') is None: warung.pop('id', None)
    ...
    else: supabase_request("warung", method="POST", data=warung)
def delete_warung(warung_id): supabase_request(f"warung?id=eq.{warung_id}", method="DELETE")
def get_next_id():
...
def tampilkan_map(warung_list):
    center_lat, center_lon = -6.2186, 106.7012
    try:
        ...
        for w in warung_list:
            ...
            if lat is not None and lon is not None:
                ...
                status = w.get('status', '')
                if "Underrated" in status: color, icon = 'purple', 'star'
                elif "Hidden Gem" in status: color, icon = 'blue', 'info-sign'
                elif "Legend" in status: color, icon = 'orange', 'fire'
                elif "Viral" in status: color, icon = 'red', 'bullhorn'
                else: color, icon = 'green', 'ok'
                popup_text = f"<b>{w['nama']}</b><br>📍 {w['lokasi']}<br>💰 {w['harga']}<br>⭐ {w['rating']}<br>🏷️ {status}"
                folium.Marker(location=[lat, lon], popup=folium.Popup(popup_text, max_width=300), tooltip=w['nama'], icon=folium.Icon(color=color, icon=icon, prefix='glyphicon')).add_to(m)
        return m
    except: return None

WILAYAH_CILEDUG = ["Semua Ciledug Raya","Karang Mulya","Karang Timur","Karang Sari","Sudimara Barat","Sudimara Timur","Sudimara Selatan","Sudimara Jaya","Paninggilan","Paninggilan Utara","Tajur","Parung Serab","Larangan Selatan","Larangan Utara","Larangan Indah","Petukangan","Jurang Mangu","Parigi","Pondok Kacang","Pondok Betung","Ciledug"]
KATEGORI = ["Mie Ayam","Nasi Goreng","Bakso","Sate","Nasi Uduk","Gado-gado","Soto","Padang","Pecel Ayam","Pecel Lele","Ayam Goreng","Seafood","Bubur Ayam","Ketupat Sayur","Nasi Bebek","Lainnya"]
STATUS_WARUNG = ["💎 Underrated","⭐ Hidden Gem","🔥 Legend","👍 Review Bagus","📱 Viral"]

def deteksi_kategori_dari_prompt(prompt):
    prompt_lower = prompt.lower()
    keyword_map = {"nasi bebek":"Nasi Bebek","bebek":"Nasi Bebek","mie ayam":"Mie Ayam","mie":"Mie Ayam","nasi goreng":"Nasi Goreng","bakso":"Bakso","sate":"Sate","nasi uduk":"Nasi Uduk","uduk":"Nasi Uduk","gado-gado":"Gado-gado","gado":"Gado-gado","soto":"Soto","padang":"Padang","pecel ayam":"Pecel Ayam","pecel lele":"Pecel Lele","lele":"Pecel Lele","ayam goreng":"Ayam Goreng","bubur ayam":"Bubur Ayam","bubur":"Bubur Ayam","ketupat":"Ketupat Sayur"}
    for keyword, kategori in keyword_map.items():
        if keyword in prompt_lower: return kategori
    return None
...
def cari_warung(lokasi_user, prompt_user):
    ...
    for warung in st.session_state.warung_ciledug:
        cocok = True
        if lokasi_user != "Semua Ciledug Raya" and lokasi_user.lower() not in warung["lokasi"].lower(): cocok = False
        if kategori_terdeteksi and warung["kategori"].lower() != kategori_terdeteksi.lower(): cocok = False
        if cocok: semua_warung.append(warung)
    urutan_prioritas = {"💎 Underrated":1,"⭐ Hidden Gem":2,"🔥 Legend":3,"👍 Review Bagus":4,"📱 Viral":5}
...
def panggil_ai(api_key, data_warung, prompt_user, lokasi_user):
    if not data_warung: return "Maaf, belum ada data warung yang cocok."
    prompt = f"""Kamu adalah asisten kuliner Ciledug Raya yang RAMAH, JUJUR, dan ADIL.
...
st.markdown("""
<style>
.main-title { font-size: 64px !important; font-weight: 900 !important; color: #D97706; text-align: center; margin-top: 10px; letter-spacing: -1px; text-shadow: 2px 2px 4px rgba(0,0,0,0.1); }
.sub-title { text-align: center; color: #4B5563; font-style: italic; margin-bottom: 20px; font-size: 18px; }
.footer { text-align: center; color: #6B7280; font-size: 12px; margin-top: 20px; }
.gmaps-link { display: inline-block; background: #4285F4; color: white !important; padding: 6px 14px; border-radius: 20px; text-decoration: none; font-size: 13px; margin-top: 6px; }
.gmaps-link:hover { background: #3367D6; color: white !important; }
.total-warung { text-align: center; font-size: 20px; color: #4B5563; margin-bottom: 10px; font-weight: 500; }
...
with tab1:
    ...
    with st.form("search_form"):
        ...
        prompt_user = st.text_area("💬 Mau makan apa?", placeholder="Contoh: 'Cari nasi bebek enak buat makan malam'", height=80)
    ...
    if submitted and prompt_user:
        with st.spinner("🔍 Mencari kuliner..."):
            ...
            if warung_ditemukan:
                if kategori_terdeteksi: st.info(f"🔍 Menemukan {len(warung_ditemukan)} {kategori_terdeteksi} di **{lokasi}**")
                else: st.info(f"🔍 Menemukan {len(warung_ditemukan)} kuliner di **{lokasi}**")
                ...
                for w in warung_ditemukan:
                    ...
                    if lat and lon: gmaps_link = f'<a href="https://www.google.com/maps?q={lat},{lon}" target="_blank" class="gmaps-link">🗺️ Buka di Google Maps</a>'
                    elif alamat and alamat != "Belum ada info": gmaps_link = f'<a href="https://www.google.com/maps/search/?api=1&query={urllib.parse.quote(alamat)}" target="_blank" class="gmaps-link">🗺️ Buka di Google Maps</a>'
                    else: gmaps_link = f'<a href="https://www.google.com/maps/search/?api=1&query={urllib.parse.quote(w["nama"] + " " + w["lokasi"])}" target="_blank" class="gmaps-link">🗺️ Buka di Google Maps</a>'
                    st.markdown(f"""
                ...
                if ada_koordinat:
                    try:
                        ...
                        if m: folium_static(m, width=700, height=400)
                    except: st.info("📝 Tidak ada koordinat valid.")
                else: st.info("📝 Belum ada koordinat. Tambahkan di Admin Panel.")
            else: st.warning("😔 Belum ada kuliner yang cocok.")
    elif submitted and not prompt_user: st.warning("⚠️ Tuliskan makanan yang kamu cari dulu ya!")
with tab2:
    st.markdown("### ➕ Rekomendasikan Tempat Kuliner")
    st.info("📝 Bantu warga Ciledug menemukan tempat kuliner favoritmu!")
    with st.form("tambah_form", clear_on_submit=True):
        ...
        with col_lat: lat = st.text_input("Latitude", placeholder="-6.2186")
        with col_lon: lon = st.text_input("Longitude", placeholder="106.7012")
        ...
        if submitted:
            if not nama or not lokasi or not kategori or not harga or not alamat or not jadwal:
                ...
                try: lat_val = float(lat) if lat else None
                except: lat_val = None
                try: lon_val = float(lon) if lon else None
                except: lon_val = None
                data_baru = {
                    "id": get_next_id(), "nama": nama, "lokasi": lokasi, "kategori": kategori,
...
with tab3:
    ...
    if not st.session_state.warung_ciledug: st.info("Belum ada data.")
    else:
        ...
        if pilihan:
            ...
            if warung:
                ...
                with st.form("edit_form"):
                    ...
                    with col_lat2: lat_edit = st.text_input("Latitude", value=str(warung.get('lat', '')) if warung.get('lat') else "")
                    with col_lon2: lon_edit = st.text_input("Longitude", value=str(warung.get('lon', '')) if warung.get('lon') else "")
                    ...
                    with col_save: save_clicked = st.form_submit_button("💾 Simpan", use_container_width=True)
                    with col_delete: delete_clicked = st.form_submit_button("🗑️ Hapus", use_container_width=True, type="secondary")
                    if save_clicked:
                        if not nama_edit or not lokasi_edit or not kategori_edit or not harga_edit:
                            st.warning("⚠️ Isi semua field penting!")
                        else:
                            try: lat_val = float(lat_edit) if lat_edit else None
                            except: lat_val = None
                            try: lon_val = float(lon_edit) if lon_edit else None
                            except: lon_val = None
                            warung["nama"] = nama_edit; warung["lokasi"] = lokasi_edit; warung["kategori"] = kategori_edit
                            warung["harga"] = harga_edit; warung["alamat"] = alamat_edit; warung["jam_buka"] = jam_edit
                            warung["jadwal_buka"] = jadwal_edit; warung["kontak"] = kontak_edit; warung["rating"] = rating_edit
                            warung["status"] = status_edit; warung["lat"] = lat_val; warung["lon"] = lon_val
                            warung["kelebihan"] = kelebihan_edit; warung["kekurangan"] = kekurangan_edit
                            warung["review_warga"] = review_edit; warung["ditambahkan_oleh"] = "Admin (Edited)"
...
<p style="font-size: 13px; color: #78350F;">Aplikasi ini 100% gratis untuk warga Ciledug Raya.</p>
